...
class ModelEvaluator(object):
    ...
    def evaluate(self, model, device,epoch=None):
        ...
        if epoch is None or epoch % self.epochs_per_eval == 0:
            ...
            for data_loader in self.data_loaders:
                phase_metrics, phase_curves = self._eval_phase(model, data_loader, data_loader.phase, device, table_data)
    ...
    def _eval_phase(self, model, data_loader, phase, device,table):
        ...
        with tqdm(total=num_examples, unit=' ' + phase) as progress_bar:
            for inputs, targets_dict in data_loader:
                ...
                with torch.no_grad():
                    ...
                    cls_logits = model.forward(inputs.to(device),table_data.to(device))
    ...
    def _get_summary_dicts(self, data_loader, phase, device, probs=None, keys=None, loss_meter=None):
        ...
        if probs is not None:
            ...
            for i in range(len(probs)):
                ...
                if labels[i]==flag:
                    sum_correct+=1
                if probs[i]<0.5 and labels[i]==0:   
                    TN+=1
                elif probs[i]>0.5 and labels[i]==1:
                    TP+=1
                elif probs[i]<0.5 and labels[i]==1: 
                    FN+=1
                elif probs[i]>0.5 and labels[i]==0:  
                    FP+=1
            accuracy=sum_correct/len(probs)
